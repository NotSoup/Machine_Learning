...
def run_step_2():
    ...
    for k in k_sweep:
        ...
        random_proj = GaussianRandomProjection(n_components=k, random_state=90210)
        data_projected = random_proj.fit_transform(data_encoded)

        cluster_labels = data_projected.predict(data_encoded)

        silhouette_scores.append(silhouette_score(data_encoded, cluster_labels))
        bic_scores.append(data_projected.bic(data_encoded))
    ...
    plt.grid()
    plt.savefig(f"./plots/RM_bic_{letter}.png", dpi=300)
    # plt.show()
    ...
    plt.grid()
    plt.savefig(f"./plots/RM_sil_{letter}.png", dpi=300)
